...
describe("WsMessage", () => {
  ...
  describe("WsMessage.fromWebSocketString", () => {
    test("should be able to parse a message from a web socket string generated by toWebSocketString", () => {
      ...
      const parsedWsMessage = parsedMessage as WsMessage;
      expect(parsedWsMessage).toBeDefined();
      expect(parsedWsMessage.id).toBe(originalMessage.id);
      expect(parsedWsMessage.type).toBe(originalMessage.type);
      expect(parsedWsMessage.roomId).toBe(originalMessage.roomId);
      expect(parsedWsMessage.sender).toBe(originalMessage.sender);
      expect(parsedWsMessage.ts).toEqual(originalMessage.ts);
      expect(parsedWsMessage.content).toBe(originalMessage.content);
      expect(parsedWsMessage.metadata).toEqual(originalMessage.metadata);
    });

    test("should return a WsMessageError if the message is not JSON", () => {
      const message = "not a JSON string";
      const [error, parsedMessage] = WsMessage.fromWebSocketString(message);
      expect(error).toBeDefined();
      expect(error).toBeInstanceOf(WsMessageError);
      const wsMessageError = error as WsMessageError;
      expect(wsMessageError.code).toBe(WsMessageErrorCodes.MALFORMED_MESSAGE);
      expect(wsMessageError.message).toBe(
    ...
    test("should return a WsMessageError if the message is JSON but an invalid schema", () => {
      ...
      expect(error).toBeInstanceOf(WsMessageError);
      const wsMessageError = error as WsMessageError;
      expect(wsMessageError.code).toBe(
        WsMessageErrorCodes.INVALID_MESSAGE_SCHEMA
      );
      expect(wsMessageError.message).toBe('Schema error for "ts": Required');
      expect(parsedMessage).toBeNull();
    });
    ...
    test("should be able to infer the roomId from the server", () => {
      ...
      const parsedWsMessage = parsedMessage as WsMessage;
      expect(parsedWsMessage.roomId).toBe(ROOM_ID);
    });
    ...
    test("should be able to infer the sender from the server", () => {
      ...
      const parsedWsMessage = parsedMessage as WsMessage;
      expect(parsedWsMessage.sender).toBe(SENDER);
    });
  });
});
